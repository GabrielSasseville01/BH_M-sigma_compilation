...
# Fit the Stan model
stan_fit = stan('model.stan', data = stan_data, iter = 2000, chains = 4, seed = 1, init=0)
...
gamma0_post_ulimit = posterior_samples$b_Intercept_ulimit
gamma1_post_ulimit = posterior_samples$b_ulimit
...
stan_samples_ulimit = data.frame(beta0_post, beta1_post, gamma0_post_ulimit, gamma1_post_ulimit, row.names=NULL)

write.table(stan_samples_ulimit, './samples_ulimit.txt', row.names=FALSE, col.names=FALSE, fileEncoding = 'UTF-8')

stan_samples_lambda = data.frame(lambda, row.names=NULL)

write.table(stan_samples_lambda, './lambda.txt', row.names=FALSE, col.names=FALSE, fileEncoding = 'UTF-8')
